...
export class JobService {
  ...
  async deleteFiles(
    ...
    const deleteResult = await this.prisma.jobFile.deleteMany({
      where: {
        jobId,
        fileKey: {
          in: fileKeys,
    ...
    if (job && job.templateJson) {
      const updatedTemplateJson = this.removeDeletedFilesFromTemplateJson(
        job.templateJson,
        fileKeys,
      );
      ...
      await this.prisma.job.update({
        where: { id: jobId },
        data: {
          templateJson: updatedTemplateJson as Prisma.InputJsonValue,
        },
      });
    }
  ...
  private removeDeletedFilesFromTemplateJson(
    templateJson: unknown,
    ...
    const deletedKeysSet = new Set(deletedFileKeys);
    ...
    // Check if it's grouped format or flat format
    const isGroupedFormat =
      templateJson.length > 0 &&
      typeof templateJson[0]?.groupName === 'string' &&
      Array.isArray(templateJson[0]?.fields);

    if (isGroupedFormat) {
      // Grouped format: clean fields within each group
      return templateJson.map((group: any) => {
        ...
        return {
          ...
          fields: Array.isArray(group.fields)
            ? group.fields.map(cleanField)
            : group.fields,
        };
      });
    } else {
      // Flat format: clean each field directly
      return templateJson.map(cleanField);
    }
  }
